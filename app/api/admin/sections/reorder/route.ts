...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify all section IDs exist and belong to admin's restaurant
    const sectionIds = validation.data.items.map(item => item.id)
    ...
    const existingIds = new Set(existingSections.map(sec => sec.id))
    const missingIds = sectionIds.filter(id => !existingIds.has(id))
    
    if (missingIds.length > 0) {
      return NextResponse.json(
        { 
          error: 'Some section IDs do not exist or belong to another restaurant',
          missingIds,
        },
        { status: 400 }
      )
